...
void set_state(PEOPLE *person, int new_state)
{
   ...
   else // symptomatic or asymptomatic infection
   {
      ...
      person->day_infection_upper  = min(person->day_infection_upper, community[h].day_epi_stop);

      //person->day_infection_lower = max(person->day_ill - cfg_pars.max_incubation, community[h].day_epi_start);
      //person->day_infection_upper  = person->day_ill - cfg_pars.min_incubation;
      //printf("day_infection_lower=%d  day_infection_upper=%d\n", 
      //        person->day_infection_lower, person->day_infection_upper);
      if(person->day_infection_upper < community[h].day_epi_start ||
        person->day_infection_upper < person->day_infection_lower)
      {
         printf("person %d in community %d has an upper bound of infection day (%d) before day_epi_start (%d)\n",
                 person->id, community[h].id, person->day_infection_upper, community[h].day_epi_start);
         printf("Try reduce the incubaiton period, extend day_epi_start to an earlier date for this community,\n");
         printf("or change the range of possible dates of illness onset in the imputation file for this person\n");
         exit(0);
      }
   }
}
                           ...
                           double *par_effective, STATE *head) 
{
   ...
   while(n < n_sample & ptr_stat != NULL)
   {
      for(m=0; m<community->size_impute; m++)
      {
         ...
         //printf("Sampling states for person %d\n", person->id);
         ...
         {
            //printf("choice %d: state=%d\n", l, person->possible_states[0][l]);
            set_state(person, l);
            if(cfg_pars.adjust_for_left_truncation == 1)   
            {
               if(cfg_pars.preset_index == 0)  
               {
                  set_index_cases(community);
                  if(cfg_pars.use_index_cases_to_improve_b == 1) update_c2p_group_single_community(community);
               }   
         ...
         //printf("sampled state %d  log_L_ini=%e\n", k, log_L_ini);
      }
...
void generate_list_states(COMMUNITY *community, double *par_effective)
{
   ...
   for(count=0; count < community->size_possible_states; count++)
   {
      ...
      for(m=0; m<community->size_impute; m++)
      {
         ...
         //printf("day_ill=%d  day_infection_lower=%d day_infection_upper=%d  day_infection=%d\n", 
         //  person->day_ill, person->day_infection_lower, person->day_infection_upper, person->day_infection);
      }
      if(cfg_pars.adjust_for_left_truncation == 1)   
      {
         if(cfg_pars.preset_index == 0)  
...
double likelihood(double *par_effective, double *extra_par)
{
   ...
   double log_L_cur, log_L_numerator, log_L_denominator;
   STATE *ptr_stat;
   PEOPLE *person;

   //find earliest index case illness onset day and latest follow-up day in each c2p phase
   // as index cases may be changed in set_index() function, this should be redone in set_index() as well.
   ...
   if(community[h].size > 0 && community[h].ignore == 0)
   {
      ...
      {
         log_L_cur = community_log_likelihood(community + h, par_effective);
         log_L += log_L_cur;
         if(cfg_pars.adjust_for_left_truncation == 1 && cfg_pars.use_index_cases_to_improve_b == 1)
         {
            id_c2p_group = community[h].c2p_group;
            if(community[h].earliest_idx_day_ill > cfg_pars.c2p_group[id_c2p_group].earliest_idx_day_ill)
            {
               log_L_numerator = community_log_likelihood_IdxAdjust_numerator(community + h, par_effective);
               log_L_denominator = community_log_likelihood_IdxAdjust_denominator(community + h, par_effective);
               log_L +=  log_L_numerator - log_L_denominator;
               //printf("Community %d: log_L_cur=%e, log_L_numerator=%e, log_L_denominator=%e, log_L=%e\n", h, log_L_cur, log_L_numerator, log_L_denominator, log_L);
               //exit(0);
            }   
         }
      ...
      else if(community[h].size_possible_states > 1 && community[h].size_possible_states < cfg_pars.min_size_MCEM)
      {
         ...
         while(ptr_stat != NULL)
         {
            ...
            if(cfg_pars.adjust_for_left_truncation == 1 && cfg_pars.preset_index == 0)
            {
                ...
                if(cfg_pars.use_index_cases_to_improve_b == 1) update_c2p_group_single_community(community+h);
            }
            ...
            if(cfg_pars.adjust_for_left_truncation == 1 && cfg_pars.use_index_cases_to_improve_b == 1)
            {
               ...
               if(community[h].earliest_idx_day_ill > cfg_pars.c2p_group[id_c2p_group].earliest_idx_day_ill)
               {
                  log_L_numerator = community_log_likelihood_IdxAdjust_numerator(community + h, par_effective);
                  log_L_denominator = community_log_likelihood_IdxAdjust_denominator(community + h, par_effective);
                  ptr_stat->log_L_cur += log_L_numerator - log_L_denominator;
               }
            }
        ...
        community[h].size_possible_states < cfg_pars.min_size_MCEM)
     {
        ...
        while(ptr_stat != NULL)
        {
           ...
           if(par_effective != NULL)
           {
              ...
              {
                 ptr_stat->log_L_cur += community_log_likelihood_IdxAdjust_numerator(community + h, par_effective) -
                                        community_log_likelihood_IdxAdjust_denominator(community + h, par_effective);
              }
...
int E_step_importance(double *par_effective_cur, double *log_L, MATRIX *score, MATRIX *info)
{
   ...
   int n_par, id_c2p_group; 
   int error, error1, error2;
   double log_L_grp, weight, sum_weight, weight_offset;
   double log_L_tmp1, log_L_tmp2;
   double log_L_denominator, log_L_numerator;
   MATRIX first, second, score_grp, info_grp;
   MATRIX first_tmp1, first_tmp2, second_tmp1, second_tmp2;
   MATRIX first_dif, second_dif;
   PEOPLE *person;
   STATE *ptr_stat;
   FILE *file;
   ...
   initialize_matrix(&first_tmp1);
   initialize_matrix(&first_tmp2);
   initialize_matrix(&second_tmp1);
   initialize_matrix(&second_tmp2);
   initialize_matrix(&first_dif);
   initialize_matrix(&second_dif);
   ...
   inflate_matrix(&first_tmp1, n_par, 1, 0);
   inflate_matrix(&first_tmp2, n_par, 1, 0);
   inflate_matrix(&second_tmp1, n_par, n_par, 0);
   inflate_matrix(&second_tmp2, n_par, n_par, 0);
   inflate_matrix(&first_dif, n_par, 1, 0);
   inflate_matrix(&second_dif, n_par, n_par, 0);
   ...
   if(community[h].size > 0 && community[h].ignore == 0)
   {
      ...
      printf("community %d: baseline day=%d idx_day_ill=%d\n", h, cfg_pars.c2p_group[j].earliest_idx_day_ill - cfg_pars.max_incubation + 1, community[h].earliest_idx_day_ill);
      for(l=0; l<community[h].size; l++)
      {
         i = community[h].member[l];
         printf("person %d: infection=%d  day_ill=%d\n", i, people[i].infection, people[i].day_ill);
      }*/  
      if(community[h].size_impute == 0)
      {
         log_L_grp = 0.0;
         reset(&score_grp, 0.0);
         reset(&info_grp, 0.0);
         //printf("community %d: ", h);
         error = community_derivatives(community+h, par_effective_cur, &log_L_grp, &first, &second);
         if(error == 1)  goto end;
         //printf("log_L_grp=%e\n", log_L_grp);
         //fmprintf(&first);
         //fmprintf(&second);
         if(cfg_pars.adjust_for_left_truncation == 1 && cfg_pars.use_index_cases_to_improve_b == 1)
         {
            ...
            if(community[h].earliest_idx_day_ill > cfg_pars.c2p_group[id_c2p_group].earliest_idx_day_ill)
            {
               error1 = community_derivatives_IdxAdjust_numerator(community + h, par_effective_cur, &log_L_tmp1, &first_tmp1, &second_tmp1);
               //printf("log_L_tmp1=%e\n", log_L_tmp1);
               error2 = community_derivatives_IdxAdjust_denominator(community + h, par_effective_cur, &log_L_tmp2, &first_tmp2, &second_tmp2);
               //printf("log_L_tmp2=%e\n", log_L_tmp2);
               /*if(h == 169)
               {
                  printf("community %d: c2p earliest idx=%d baseline day=%d idx_day_ill=%d\n", h, 
                          cfg_pars.c2p_group[j].earliest_idx_day_ill,
                          cfg_pars.c2p_group[j].earliest_idx_day_ill - cfg_pars.max_incubation + 1, community[h].earliest_idx_day_ill);
                  printf("error1=%d  error2=%d\n", error1, error2);
                  printf("log_L_tmp1=%e  log_L_tmp2=%e  diff=%e\n", log_L_tmp1, log_L_tmp2, log_L_tmp1 - log_L_tmp2);
                  log_L_numerator = community_log_likelihood_IdxAdjust_numerator(community + h, par_effective_cur);
                  log_L_denominator = community_log_likelihood_IdxAdjust_denominator(community + h, par_effective_cur);
                  printf("log_L_grp=%e, log_L_numerator=%e, log_L_denominator=%e  diff=%e\n", log_L_grp, log_L_numerator, log_L_denominator, log_L_numerator-log_L_denominator);
                  addition(&first_tmp1, 1.0, &first_tmp2, -1.0, &first_dif);
                  addition(&second_tmp1, 1.0, &second_tmp2, -1.0, &second_dif);
                  fmprintf(&first_dif);
                  fmprintf(&second_dif);
                  exit(0);
               }*/
               ...
               {
                  printf("Error in E_step_importance() function, observed community=%d: numerator error=%d  denominator error=%d\n", h, error1, error2);
                  exit(0);
               }   
               log_L_grp += log_L_tmp1 - log_L_tmp2;
               addition(&first, 1.0, &first_tmp1, 1.0, &first);
               addition(&first, 1.0, &first_tmp2, -1.0, &first);
               addition(&second, 1.0, &second_tmp1, 1.0, &second);
               addition(&second, 1.0, &second_tmp2, -1.0, &second);
            }
         }
         addition(&score_grp, 1.0, &first, 1.0, &score_grp);
         addition(&info_grp, 1.0, &second, -1.0, &info_grp);
      ...
      {
         //printf("EM communty: %d size_possible_states=%d\n", h, community[h].size_possible_states);
         log_L_grp = 0.0;
         reset(&score_grp, 0.0);
         reset(&info_grp, 0.0);
         ptr_stat = community[h].list_states;
         ...
         while(ptr_stat != NULL)
         {
            //if(h == 67 || h == 99) printf("state setting %d:\n", count);
            for(i=0; i<community[h].size_impute; i++)
            {
               person = people + community[h].member_impute[i];
               l = ptr_stat->states[i];
               //if(h == 67 || h == 99)  printf("person %d: state=%d\n", person->id, person->possible_states[0][l]);
               //printf("choice %d: state=%d\n", l, person->possible_states[0][l]);
               set_state(person, ptr_stat->states[i]);
            }   
            if(cfg_pars.adjust_for_left_truncation == 1 && cfg_pars.preset_index == 0)
            ...
            if(error == 0)
            {
               if(cfg_pars.adjust_for_left_truncation == 1 && cfg_pars.use_index_cases_to_improve_b == 1)
               {
                  ...
                  if(community[h].earliest_idx_day_ill > cfg_pars.c2p_group[id_c2p_group].earliest_idx_day_ill)
                  {
                     ...
                     {
                        ptr_stat->log_L_cur += log_L_tmp1 - log_L_tmp2;
                        addition(&first, 1.0, &first_tmp1, 1.0, &first);
                        addition(&first, 1.0, &first_tmp2, -1.0, &first);
                        addition(&second, 1.0, &second_tmp1, 1.0, &second);
                        addition(&second, 1.0, &second_tmp2, -1.0, &second);
                     ...
                     {
                        printf("Error in E_step_importance() function, EM community=%d: numerator error=%d  denominator error=%d\n", h, error1, error2);
                        exit(0);
                     }   
                  }
               }
               ...
               //if(h == 99)  printf("State %d:  weight=%e  sum_weight=%e  log_L_pre=%e  log_L_cur=%e  log_L_grp=%e\n", count, weight, sum_weight, ptr_stat->log_L_pre, ptr_stat->log_L_cur, log_L_grp);
            }
            ptr_stat = ptr_stat->next;
            count++;
         }
      ...
      {
         //printf("MCEM communty: %d size_possible_states=%d\n", h, community[h].size_possible_states);
         log_L_grp = 0.0;
         reset(&score_grp, 0.0);
         reset(&info_grp, 0.0);
         sum_weight = 0.0;
            ...
            if(error == 0)
            {
               if(cfg_pars.adjust_for_left_truncation == 1 && cfg_pars.use_index_cases_to_improve_b == 1)
               {
                  ...
                  if(community[h].earliest_idx_day_ill > cfg_pars.c2p_group[id_c2p_group].earliest_idx_day_ill)
                  {
                     ...
                     {
                        printf("Error in E_step_importance() function, MCEM community=%d: numerator error=%d  denominator error=%d\n", h, error1, error2);
                        exit(0);
                     }   
                  }
               }
   ...
   deflate_matrix(&first_tmp1);
   deflate_matrix(&first_tmp2);
   deflate_matrix(&second_tmp1);
   deflate_matrix(&second_tmp2);
   deflate_matrix(&first_dif);
   deflate_matrix(&second_dif);
   return(error);
}
...
int M_step(double *par_effective, double *max_log_L, int optimization_method)
{
   ...
   else
   {
      ...
      while( converge == 0 && loop < max_iter)
      {
         ...
         //printf("par_effective_old:\n");
         //show_par_effective(par_effective, NULL);
         //printf("par_effective_new:\n");
...
{
   int i, j, h, k, l, m, n, count, id_c2p_group;
   int n_par, n_par_equiclass, n_b_mode, n_p_mode, n_q_mode, n_u_mode; 
   int error, error1, error2, any_negative_variance;
   double *der, log_L_grp, weight, sum_weight;
   double *log_L_cur, *log_L_ini, log_L_tmp1, log_L_tmp2;
   ...
   if(aux_community != NULL)
   {
      ...
      else
      {
         ...
         make_1d_array_double(&log_L_ini, size_sample_states, 0.0);
         for(h=0; h<n_community; h++)
         if(community[h].size_possible_states > 1 && community[h].ignore == 0)
         {
            k = 0;
      ...
      if(community[h].size_impute == 0)
      {
         ...
         if(error == 0)
         {
            if(cfg_pars.adjust_for_left_truncation == 1 && cfg_pars.use_index_cases_to_improve_b == 1)
            {
               ...
               if(community[h].earliest_idx_day_ill > cfg_pars.c2p_group[id_c2p_group].earliest_idx_day_ill)
               {
                  error1 = community_derivatives_IdxAdjust_numerator(community + h, par_effective, &log_L_tmp1, &first_tmp1, &second_tmp1);
                  error2 = community_derivatives_IdxAdjust_denominator(community + h, par_effective, &log_L_tmp2, &first_tmp2, &second_tmp2);
                  if(error1 == 0 && error2 == 0)
                  {
                     log_L_grp += log_L_tmp1 - log_L_tmp2;
                     addition(&first, 1.0, &first_tmp1, 1.0, &first);
                  ...
                  {
                     printf("Error in covariance() function, EM-MCEM community=%d: numerator error=%d  denominator error=%d\n", h, error1, error2);
                     exit(0);
                  }   
               }
            }   
   ...
   any_negative_variance = 0;
   for(i=0; i<n_par_equiclass; i++)
   {
      if(sub_inv.data[i][i] < 0)   any_negative_variance = 1;
   }   

   if(any_negative_variance == 1)
   {
      printf("Variance estimates are negative for some parameters:\n");
      fmprintf(&sub_inv);
      printf("If these parameters reach search bounds, we set their variances and covariances be 0. CAUTION: their confidence intervals are not interpretable.\n");
      printf("For other parameters, it is recommended to use boostrap to verify their variances and confidence intervals\n");
   }
   for(i=0; i<n_par_equiclass; i++)
   {
      if(sub_inv.data[i][i] < 0)
      {
         if(par_effective[i] <= cfg_pars.lower_search_bound[i] ||
            par_effective[i] >= cfg_pars.upper_search_bound[i])
         {
            for(j=i; j<n_par_equiclass; j++)
               sub_inv.data[i][j] = sub_inv.data[j][i] = 0;
         }
      }
   }   
   //parse the Fisher information to original parameters
   ...
   else
   {
      ...
      if(n_need_MCEM > 0)
      {
         ...
         for(i=0; i<cfg_pars.n_burnin_iter; i++)
         {
            ...
            if(community[h].size_possible_states >= cfg_pars.min_size_MCEM)
            {
               //printf("generate sample states for community%d\n", h);
               generate_sample_states(community+h, size_sample_states, cfg_pars.n_burnin_sampling, par_effective, community[h].sample_states); 
            }
         ...
         TIME_SAMPLING_CPU = T2 - T1;
         if(cfg_pars.silent_run == 0)  
            printf("It took %ld seconds (%ld cpu clocks) to generate MCMC importance samples for MCEM\n", TIME_SAMPLING_SEC, TIME_SAMPLING_CPU);
      }
     
      if(cfg_pars.silent_run == 0)
         printf("\n\nBegin EM-MCEM iterations with fixed sample size\n");
      ...
      TIME_ITERATION_CPU = T2 - T1;
      if(cfg_pars.silent_run == 0)  
         printf("It took %ld seconds and %d iterations to finish the EM-MCEM iterations\n", TIME_ITERATION_SEC, loop);
      ...
      if( converge == 0)
      {
          printf("EM-MCEM iterations with increasing sample size does not converge\n");
          error = 1;
          goto end;
      }
      //t2 = time(NULL);
...
int estimation(int id_inc, int id_inf, int id_time, double *est, double *logL, MATRIX *var, MATRIX *var_logit)
{
  ...
  // For case-ascertained study, especially when housheold follow-up period is relative short compared to infectious period,
  // we can improve estimaiton of b using informaiton from the sequence of symptom onsets of index cases.
  // As households may come from multiple epidemic seasons, we assign households to these seasons.
  // For each season, we track some key days such as earliest index illness day, earliest epi_start, and latest epi_stop
  // across all households in that season. 
  // The season membership may change if we allow index case to change when imputing day of infecitousness onset for asymptomatic
  // infections. This may affect convergence. A good practice is prefixing index cases.
  

  //printf("optimization choice=%d\n", cfg_pars.optimization_choice);
