...
typedef struct 
{
   int earliest_idx_day_ill;
   int latest_idx_day_ill;
   int community_of_earliest_idx_day_ill;
   int community_of_latest_idx_day_ill;
} C2P_GROUP;
...
typedef struct {
        ...
        int c2p_group;// if we use index cases to improve estimation of b, we need to assgin each community to a c2p group
        int day_last_followup; // enforce a follow-up period after enrollment
...
typedef struct {
    ...
    int PreIllness_as_covariate; // whether the pre-symptom-onset vs post-symptom-onset is used as a time dependent covariate. For simulation only.
    int PreIllness_covariate_id; // This is useful if you want to compare infectivity level during incubation period vs during illness period. For simulation only.
    int Illness_as_covariate; // whether illness is used as a time independent covariate. For simulation only.
    int Illness_covariate_id; // This is useful if you want to compare infectivity level of symptomatic cases vs asymptomatic cases. For simulation only.
    int RxIllness_as_covariate; // whether treat symptomatic cases with antivirals or not. For simulation only.
    int RxIllness_covariate_id; // which time-dep covariate encodes antiviral treatment. For simulation only.
    int RxIllness_index_only; // Treat index cases only or all symptomatic cases. For simulation only.
    int RxIllness_duration; // duration of antiviral treatment, counting from symptom onset. For simulation only.
    double RxIllness_prob; // probability of antiviral treatment. For simulation only.
    ...
    int simulation_only; //Only perform simulations, no estimation
    ...
    int idx_initiated_followup_in_simulation;
    int followup_duration_after_idx_in_simulation;
    int use_index_cases_to_improve_b;
    int n_c2p_group;
    C2P_GROUP *c2p_group; // c2p_phase definess time intervals within each of which we use the arrival times of households (essentially index illness onset)
                         // to improve the estimation of b, especially when the follow-up period is relatively short compared to the infectious period in case-ascertained design.
