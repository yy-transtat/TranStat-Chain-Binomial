...
int core(int id_inc, int id_inf, int id_time)
{
  ...
  int h, i, j, k, l, n, m, r, t, len_inc;
  long n_record; 
  int n_iter, n_sym_inf, n_asym_inf;
  ...
  printf("use index cases to improve b=%d\n", cfg_pars.use_index_cases_to_improve_b);
  printf("n_c2p phase=%d\n", cfg_pars.n_c2p_phase);
  for(i=0; i<cfg_pars.n_c2p_phase; i++)
  {
     printf("%d  %d\n", cfg_pars.c2p_phase[i].day_start, cfg_pars.c2p_phase[i].day_stop);
     printf("\n");
  }
  printf("dynamically update c2p phase=%d\n", cfg_pars.dynamically_update_c2p_phase);
  goto end;*/
  ...
  else
  {
     ...
     for(h=0; h < n_community; h++)
     {
        ...
        community[h].day_last_followup = MISSING;
        ...
        fscanf(file, "%d", &community[h].day_last_followup);  
        fscanf(file, "%d", &community[h].c2p_group);  
  ...
  if(people[i].ignore == 0)
  {
     ...
     if(cfg_pars.simulation == 0 && cfg_pars.EM == 0)
     {
        if(people[i].day_ill > community[h].day_epi_stop && cfg_pars.adjust_for_right_censoring == 1)
        {
        ...
        { 
           ...
           people[i].day_infection_upper  = min(people[i].day_infection_upper, community[h].day_epi_stop);
  ...
  if(n_time_ind_covariate > 0)
  {
     ...
     rewind(file); 
     printf("n_time_ind_covariate=%d,  n_record=%ld\n", n_time_ind_covariate, n_record);
  ...
  if(n_time_dep_covariate > 0)
  {
     ...
     n_record = get_size(file);
     rewind(file); 
     printf("n_time_dep_covariate=%d,  n_record=%ld\n", n_time_dep_covariate, n_record);
     for(m=0; m<n_record; m++)
     {
  ...
  **************************************************************/
  ...
  /*************************************************************
  Begin iterations for simulation. If this is not a simulation, 
  ...
  for(ITER=0; ITER<n_iter; ITER++)
  {
     ...
     { 
        ...
        if(cfg_pars.output_simulation_data == 1) //output simulated data (only with time-independent covariates) for regression analysis
        {
           ...
           for(i=0,person=people; i < p_size; i++,person++)
           {
              fprintf(file, "%12d  %12d  %d  %d  %12d  %d  %12d", 
                            person->id, person->community, person->idx, person->infection, 
                            person->day_infection, person->symptom, person->day_ill); 
              for(j=0; j<n_time_ind_covariate; j++)
                 fprintf(file, "  %12.6f",  person->time_ind_covariate[j]);
              fprintf(file, "\n");
           }
           fclose(file);      
        }
        ...
        if(cfg_pars.adjust_for_left_truncation == 1 && cfg_pars.use_index_cases_to_improve_b == 1) 
           update_c2p_group_all_communities();
        printf("simulation_only=%d\n", cfg_pars.simulation_only);
        if(cfg_pars.simulation_only == 1)  goto loop_end;
     }   
     printf("check0\n");
     ...
     */
     ...
     // Therefore, we disable above code, and DO NOT TRY TO INDENTIFY CO-PRIMARY CASES BASED ON SYMPTOM ONSETS. If user want some subjects
     ...
     // For case-ascertained design, identify index cases, create a chain of index cases, and find earliest and last illness onsets in the community.
     // Note this has been done in the simulate() function if performing simulation.
     if(cfg_pars.simulation == 0 && cfg_pars.adjust_for_left_truncation == 1)
     {
        for(h=0; h<n_community; h++)   
        {
           ...
           // For case-ascertained design, families without index cases or with only index cases are usually excluded from analysis.
           // However, there are several situations these families should not be excluded:
           // (1) If EM-MCEM is to be used, we cannot exclude these families because index cases could be generated when we sample possible states in these families.
           //     Example: a family has only an asymptomatic infection and no symptomatic infection, and the day_ill of this person will be set to MISSING when pop.dat is read in.
           //     TranStat will not detect this peron as an index case (unless user set this infection as index case and set preset_index=1),
           //     and thus size_idx=0. When EM-MCEM is running, this person will be set as the index case as he is the only person infected in this family.
           // (2) If we use arrival times of index cases to improve estimation of b, then households with only index cases should be retained.
           
           if(!(cfg_pars.preset_index == 0 && cfg_pars.EM == 1))
           {
              if(community[h].size_idx == 0 || (community[h].size_idx == community[h].size && cfg_pars.use_index_cases_to_improve_b == 0))
              {
        ...
        // update earliest and lateset index case sympom onset days for each c2p_group.
        if(cfg_pars.use_index_cases_to_improve_b == 1) update_c2p_group_all_communities();
     }
     ...
     if(cfg_pars.EM == 1)
     {
        if(cfg_pars.simulation == 0)
        {
           ...
           fclose(file);
        } // end of if(cfg_pars.simulation == 0)
        else
        {
           for(i=0; i<p_size; i++)
           {
              // for simulation, people[i].day_ill records the true day of infectiousness onset for asymtompatic cases.
              if(people[i].ignore != 1 && people[i].infection == 1 && people[i].symptom == 0)
              {
                 h = people[i].community;
                 //we assume weekly test so that infectiousness onset day of each asymptomatic case
                 //can be narrowed down to the week it falls in.  
                 k = (people[i].day_ill-community[h].day_epi_start)/test_interval_length;
                 possible_asym_start = max(community[h].day_epi_start + k * test_interval_length, community[h].day_epi_start + cfg_pars.min_incubation);
                 possible_asym_stop = min(community[h].day_epi_start + (k+1) * test_interval_length, community[h].day_epi_stop + cfg_pars.max_incubation);
                 ...
                 /*if(people[i].id == 368)
                 {
                    printf("k=%d  day_ill=%d\n", k, people[i].day_ill);
                    printf("possible_asym_start=%d  possible_asym_stop=%d  people[i].size_possible_states=%d \n", possible_asym_start, possible_asym_stop, people[i].size_possible_states);
                    goto end;
                 }*/   
                 make_2d_array_int(&people[i].possible_states, 2, people[i].size_possible_states, 0);
                 k = 0;
        ...
        } //end of if(cfg_pars.simulation == 1)
        ...
        if(people[i].ignore == 0 && people[i].infection == 1 && people[i].size_possible_states == 0)
        {
           ...
           if(people[i].day_infection_upper < community[h].day_epi_start)
           {
              printf("person %d in community %d has an upper bound of infection day (%d) before day_epi_start (%d)\n", 
                      people[i].id, people[i].community, people[i].day_infection_upper, community[h].day_epi_start);
              printf("Try reduce the incubaiton period or extend day_epi_start to an earlier date for this community\n");
              goto end;
           }
           if(people[i].day_infection_lower > community[h].day_epi_stop)
           {
              printf("person %d in community %d has an lower bound of infection day (%d) after day_epi_stop (%d)\n", 
                      people[i].id, people[i].community, people[i].day_infection_lower, community[h].day_epi_stop);
              printf("Try reduce the incubaiton period or extend day_epi_start to an earlier date for this community\n");
              goto end;
           }
        ...
        for(i=0; i<p_size; i++)
        {
           fprintf(file, "%d  %d  infection=%d  day_infection=%d  symptom=%d  day_ill=%d  idx=%d  infection:(lower=%d  upper=%d)  infective:(lower=%d  upper=%d)  ignore=%d\n", 
                i, people[i].community,  people[i].infection,  people[i].day_infection, 
     ...
     else // build individual-level risk history
     {
        for(i=0; i<p_size; i++)
        {
           ...
           if(person->ignore == 0)
           {
              ...
              for(t=community[h].day_epi_start; t<=community[h].day_epi_stop; t++)
              // abandon final_risk_day. we need risk profile for the whole epidemic period.
     ...
     ***********************************************************************************/
     ...
     if(estimation_error_type == 0 && cfg_pars.skip_output == 0)
     {
        ...
        for(k=0; k<n_p_mode; k++)
        {
           ...
           //printf("check: estimate SAR adjusted for covariates\n");
        ...
        if(cfg_pars.simplify_output == 1)
        {
           ...
           if(cfg_pars.print_covariance == 1)   
           for(m=0; m<n_par; m++)
           {
              for(n=m; n<n_par; n++)
              {
