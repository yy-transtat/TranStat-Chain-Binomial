...
void show_c2p_risk(PEOPLE *person, int start, int stop)
{
   ...
   for(t=start; t<=stop; t++)
   {
      r = t - community[h].day_epi_start;
      {
         ...
         while(ptr_risk != NULL)
         {
            printf("id=%d  day=%d  contact_mode=%d  offset=%f  size=%d  covariate:",
...
void show_p2p_risk(PEOPLE *person, int start, int stop)
{
   ...
   for(t=start; t<=stop; t++)
   {
      r = t - community[h].day_epi_start;
      {
         ...
         while(ptr_risk != NULL)
         {
            printf("id=%d  day=%d  contact_mode=%d  symptom=%d  infective_prob=%e offset=%f  size=%d  covariate:",
...
void create_state_chain(long n,  int m, STATE **head, STATE **rear)
{
   int i, j, k, l;
   STATE *ptr_stat;
...
/*
// this function find the earliest symptom onset day and latest follow-up day
// in each c2p phase, which is needed to improve estimaiton of b for case-ascertained design
void update_c2p_group_all_communities(void)
{
   int h; 
   int l;
   if(cfg_pars.use_index_cases_to_improve_b == 1)
   {
      for(l=0; l<cfg_pars.n_c2p_group; l++)
      {
         cfg_pars.c2p_group[l].earliest_idx_day_ill = INFINITY_INTEGER;
         cfg_pars.c2p_group[l].latest_idx_day_ill = -INFINITY_INTEGER;
         cfg_pars.c2p_group[l].earliest_day_epi_start = INFINITY_INTEGER;
         cfg_pars.c2p_group[l].latest_day_epi_stop = -INFINITY_INTEGER;
      }
      for(h=0; h<n_community; h++)
      if(community[h].size > 0 && community[h].ignore == 0)
      {
         // determine which c2p phase each community belongs to
         for(l=0; l<cfg_pars.n_c2p_group; l++)
         {
            if(community[h].earliest_idx_day_ill >= cfg_pars.c2p_group[l].day_start &&
               community[h].earliest_idx_day_ill <= cfg_pars.c2p_group[l].day_stop)
               community[h].c2p_group = l;
         }    
         // 
         l = community[h].c2p_group;
         //printf("Community=%d  c2p_group=%d  day_epi_stop=%d  latest_day_epi_stop=%d\n", h, l, community[h].day_epi_stop, cfg_pars.c2p_group[l].latest_day_epi_stop);
         if(l == MISSING) {printf("Community %d does not belong to a valid c2p phase\n", h); exit(0);}

         if(community[h].earliest_idx_day_ill < cfg_pars.c2p_group[l].earliest_idx_day_ill)
            cfg_pars.c2p_group[l].earliest_idx_day_ill = community[h].earliest_idx_day_ill;
         if(community[h].latest_idx_day_ill > cfg_pars.c2p_group[l].latest_idx_day_ill)
            cfg_pars.c2p_group[l].latest_idx_day_ill = community[h].latest_idx_day_ill;
         if(community[h].day_epi_start < cfg_pars.c2p_group[l].earliest_day_epi_start)
            cfg_pars.c2p_group[l].earliest_day_epi_start = community[h].day_epi_start;
         if(community[h].day_epi_stop > cfg_pars.c2p_group[l].latest_day_epi_stop)
         {
            cfg_pars.c2p_group[l].latest_day_epi_stop = community[h].day_epi_stop;
         }   
      }
   }
}

// if index case is changed for a community, it may also change which c2p phase it belongs to, and
// thereby affects the key dates of the related c2p phases. 
void update_c2p_group_single_community(COMMUNITY *ptr_community)
{
   int h, l, id_old, id_new;
   if(cfg_pars.use_index_cases_to_improve_b == 1 && 
      ptr_community->size > 0 && ptr_community->ignore == 0)
   {
      id_old = ptr_community->c2p_group;
      if(id_old == MISSING) {printf("Community %d does not belong to a valid c2p phase\n", ptr_community->id); exit(0);}
      // determine which c2p phase each community belongs to
      for(l=0; l<cfg_pars.n_c2p_group; l++)
      {
         if(ptr_community->earliest_idx_day_ill >= cfg_pars.c2p_group[l].day_start &&
            ptr_community->earliest_idx_day_ill <= cfg_pars.c2p_group[l].day_stop)
            ptr_community->c2p_group = l;
      }    
      id_new = ptr_community->c2p_group;

      // if the community has key dates overlapping with either the old or the new c2p phase,
      // switching this commounity's c2p phase, or just the new index illness onset itself,
      // may change the key dates of the c2p phases. We need to update those dates accordingly.
      // new c2p phase may be the same as the old one, but that doesn't matter, we may still need to update key dates of c2p phases.
      // Gowever, we do expect no need to update the key dates in most cases, i.e., a new index illness onset day does not lead to switching c2p phase,
      // and does not affect the earliest or latest index case illness onset dates of the current c2p phase. 
      if((ptr_community->earliest_idx_day_ill <= cfg_pars.c2p_group[id_old].earliest_idx_day_ill) ||
         (ptr_community->latest_idx_day_ill   >= cfg_pars.c2p_group[id_old].latest_idx_day_ill) ||
         (ptr_community->day_epi_start        <= cfg_pars.c2p_group[id_old].earliest_day_epi_start) ||
         (ptr_community->day_epi_stop         >= cfg_pars.c2p_group[id_old].latest_day_epi_stop) ||
         (ptr_community->earliest_idx_day_ill <  cfg_pars.c2p_group[id_new].earliest_idx_day_ill) ||
         (ptr_community->latest_idx_day_ill   >  cfg_pars.c2p_group[id_new].latest_idx_day_ill) ||
         (ptr_community->day_epi_start        <  cfg_pars.c2p_group[id_new].earliest_day_epi_start) ||
         (ptr_community->day_epi_stop         >  cfg_pars.c2p_group[id_new].latest_day_epi_stop))
      {
         for(l=0; l<cfg_pars.n_c2p_group; l++)
         {
            cfg_pars.c2p_group[l].earliest_idx_day_ill = INFINITY_INTEGER;
...
*/
//
// This function finds earliest and latest symptom onsets for c2p groups
// based on index case designation in each c2p group.
// c2p group is need to group communities according to epidemic seasons if
// user intends to utilize the sequential arrival times of index cases (actually infected communities)
// to provide extra information about b, the external infection force.
// This function is called by the EM-MCEM algorithm when symptom/infectiousness onset
// days are resampled.
void update_c2p_group_all_communities(void)
{
   int h,l; 
   if(cfg_pars.use_index_cases_to_improve_b == 1)
   {
      ...
      {
         l = community[h].c2p_group;
         if(l == MISSING) {printf("Community %d does not belong to a valid c2p phase\n", community[h].id); exit(0);}

         if(community[h].earliest_idx_day_ill < cfg_pars.c2p_group[l].earliest_idx_day_ill)
         {
            cfg_pars.c2p_group[l].earliest_idx_day_ill = community[h].earliest_idx_day_ill;
            cfg_pars.c2p_group[l].community_of_earliest_idx_day_ill = community[h].id;
         }
         if(community[h].latest_idx_day_ill > cfg_pars.c2p_group[l].latest_idx_day_ill)
         {
            cfg_pars.c2p_group[l].latest_idx_day_ill = community[h].latest_idx_day_ill;
            cfg_pars.c2p_group[l].community_of_latest_idx_day_ill = community[h].id;
         }
      }
   }
}

// This function adjust earliest and latest symptom onsets and associated community for a c2p group
// based on changes in index case designation in one community belonging to that c2p group.
void update_c2p_group_single_community(COMMUNITY *ptr_community)
{
   int h, l, id_c2p_grp;
   if(cfg_pars.use_index_cases_to_improve_b == 1 && 
      ptr_community->size_idx > 0 && 
      ptr_community->earliest_idx_day_ill != MISSING && 
      ptr_community->latest_idx_day_ill != MISSING && 
      ptr_community->ignore == 0)
   {
      id_c2p_grp = ptr_community->c2p_group;
      if(id_c2p_grp == MISSING) {printf("Community %d does not belong to a valid c2p group\n", ptr_community->id); exit(0);}

      // if the community has the earliest index onset in the whole c2p group
      if(ptr_community->earliest_idx_day_ill <= cfg_pars.c2p_group[id_c2p_grp].earliest_idx_day_ill)
      {
         cfg_pars.c2p_group[id_c2p_grp].earliest_idx_day_ill = ptr_community->earliest_idx_day_ill;
         cfg_pars.c2p_group[id_c2p_grp].community_of_earliest_idx_day_ill = ptr_community->id;
      }
      // if the community has the latest index onset in the whole c2p group
      else if(ptr_community->latest_idx_day_ill >= cfg_pars.c2p_group[id_c2p_grp].latest_idx_day_ill)
      {
         cfg_pars.c2p_group[id_c2p_grp].latest_idx_day_ill = ptr_community->latest_idx_day_ill;
         cfg_pars.c2p_group[id_c2p_grp].community_of_latest_idx_day_ill = ptr_community->id;
      }
      // if this community used to have the earliest or latest symtom onset in the whole c2p group
      // but not anymore after resampling states and re-designating index case in this community,
      // we need to sort out which community has the earliest or latest index onset.
      else if(cfg_pars.c2p_group[id_c2p_grp].community_of_earliest_idx_day_ill == ptr_community->id ||
              cfg_pars.c2p_group[id_c2p_grp].community_of_latest_idx_day_ill == ptr_community->id)
      {
         cfg_pars.c2p_group[id_c2p_grp].earliest_idx_day_ill = INFINITY_INTEGER;
         cfg_pars.c2p_group[id_c2p_grp].latest_idx_day_ill = -INFINITY_INTEGER;

         for(h=0; h<n_community; h++)
         if(community[h].c2p_group == id_c2p_grp && community[h].size_idx > 0 && community[h].ignore == 0)
         {
            if(community[h].earliest_idx_day_ill < cfg_pars.c2p_group[id_c2p_grp].earliest_idx_day_ill)
            {
               cfg_pars.c2p_group[id_c2p_grp].earliest_idx_day_ill = community[h].earliest_idx_day_ill;
               cfg_pars.c2p_group[id_c2p_grp].community_of_earliest_idx_day_ill = community[h].id;
            }
            if(community[h].latest_idx_day_ill > cfg_pars.c2p_group[id_c2p_grp].latest_idx_day_ill)
            {
               cfg_pars.c2p_group[id_c2p_grp].latest_idx_day_ill = community[h].latest_idx_day_ill;
               cfg_pars.c2p_group[id_c2p_grp].community_of_latest_idx_day_ill = community[h].id;
            }
         }   
      }
   }
...
// This function assign index cases for communities  according to symptom onset dates
// If EM-MCEM is used in which infectiousness onset days of asymptomatic cases are sampled,
// this function is also invoked to reassign index cases, unless user specify preset_index=1 
// in the configuration file.
// This function is called only if adjust_for_left_truncation=1 in the configuration file.
void set_index_cases(COMMUNITY *ptr_community)
{
   int i, j, k, l; 
   int earliest_day_ill;

   // remove existing designation of index cases
   ptr_community->size_idx = 0;
   if(ptr_community->idx != NULL)
   {
      free(ptr_community->idx);
      ptr_community->idx = NULL;
   }      
   ptr_community->latest_idx_day_ill = ptr_community->earliest_idx_day_ill = MISSING;
   // If index cases are given in pop.dat and user told us to fix index case designation, 
   // no need to identify who are index cases. This is true in most real data ananlyses.
   if(cfg_pars.preset_index == 1) 
   {
      for(j=0; j<ptr_community->size; j++)
      {
         i = ptr_community->member[j];
         if(people[i].idx == 1)  ptr_community->size_idx ++;
      }
      if(ptr_community->size_idx > 0)
      {
         ptr_community->idx = (int *) malloc((size_t) ptr_community->size_idx * sizeof(int));
         ptr_community->counter = 0;
         for(j=0; j<ptr_community->size; j++)
         {
            i = ptr_community->member[j];
            if(people[i].idx == 1)  
            {
               ptr_community->idx[ptr_community->counter] = i;
               ptr_community->counter++;
            }
         }
      }
      i = ptr_community->idx[0];
      ptr_community->earliest_idx_day_ill = ptr_community->latest_idx_day_ill = people[i].day_ill;
      for(j=1; j<ptr_community->size_idx; j++)
      {
         i = ptr_community->idx[j];
         if(people[i].day_ill < ptr_community->earliest_idx_day_ill)
            ptr_community->earliest_idx_day_ill = people[i].day_ill;
         if(people[i].day_ill > ptr_community->latest_idx_day_ill)
            ptr_community->latest_idx_day_ill = people[i].day_ill;
      }
   }
   // If user does not pre-specify index cases or we are simulating data.
   // We assume index cases have the earliest day_ill. If more than one community members have the earliest day_ill,
   // all of them are index cases (an alternative way is to randomly pick one case as index case, which is not programmed here). 
   ...
   {
      for(j=0; j<ptr_community->size; j++)
         people[ptr_community->member[j]].idx = 0;
   
      ptr_community->ignore = 0;
      for(j=0; j<ptr_community->size; j++)  people[ptr_community->member[j]].ignore = 0; 
      ...
      for(j=0; j<ptr_community->size; j++)
      {
         ...
         {
            if(ptr_community->earliest_idx_day_ill == MISSING) 
               ptr_community->earliest_idx_day_ill = people[i].day_ill;
            else
               ptr_community->earliest_idx_day_ill = min(ptr_community->earliest_idx_day_ill, people[i].day_ill);    
            //if(ptr_community->id > 490)
            //   printf("community=%d  person=%d  day_ill=%d earliest_day_ill=%d\n", ptr_community->id, people[i].id, people[i].day_ill, ptr_community->earliest_idx_day_ill);
         }
      }
      ptr_community->latest_idx_day_ill = ptr_community->earliest_idx_day_ill;
      //if(ptr_community->id > 490) printf("community=%d  earliest_idx_day_ill=%d  latest_idx_day_ill=%d\n", ptr_community->id, ptr_community->earliest_idx_day_ill, ptr_community->latest_idx_day_ill);
      for(j=0; j<ptr_community->size; j++)
      {
         i = ptr_community->member[j];
         if(people[i].infection == 1 && people[i].day_ill != MISSING)
         {
            ...
            //if(people[i].day_ill == ptr_community->earliest_idx_day_ill && ptr_community->size_idx == 0) //pick only one person as index case
            if(people[i].day_ill == ptr_community->earliest_idx_day_ill) //all cases with illness onset on day earliest_idx_day_ill are set as index cases
            { 
               people[i].idx = 1;
               ptr_community->size_idx ++;
            }   
         }
      }
      if(ptr_community->size_idx > 0)
