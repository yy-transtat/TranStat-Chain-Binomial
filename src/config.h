...
CFG_PARS get_cfg_pars(FILE *file)
{
  ...
  cfg_pars.simulation = cfg_pars.n_simulation = cfg_pars.simulation_only = cfg_pars.EM = 0;
  ...
  cfg_pars.PreIllness_as_covariate = cfg_pars.Illness_as_covariate = 0;
  cfg_pars.use_index_cases_to_improve_b = cfg_pars.n_c2p_group = 0;
  ...
  while( ! feof(file))
  {
     ...
     if(c == '#')
     {
        ...
        {
           if(strcmp(st, "pre-post-illness-onset-as-a-time-dependent-covariate-for-infectivity") == 0)
           {
              fscanf(file,"%d:", &cfg_pars.PreIllness_as_covariate);
              if(cfg_pars.PreIllness_as_covariate == 1)
              { 
                 fscanf(file,"%d", &cfg_pars.PreIllness_covariate_id);
                 if(cfg_pars.PreIllness_covariate_id > cfg_pars.n_time_dep_covariate)
                 {
                    printf("PreIllness_as_covariate=%d PreIllness_covariate_id=%d\n",  cfg_pars.PreIllness_as_covariate, cfg_pars.PreIllness_covariate_id);
                    printf("Please input a valid time dependent covariate for pre-illness indicator\n");
                    exit(0);
                 }
              }   
           }      
        ...
        if(cfg_pars.n_time_dep_covariate > 0)
        {
           if(strcmp(st, "antiviral-treatment-as-a-time-dependent-covariate-for-infectivity") == 0)
           {
              fscanf(file,"%d:", &cfg_pars.RxIllness_as_covariate);
              if(cfg_pars.RxIllness_as_covariate == 1)
              { 
                 fscanf(file,"%d  %lf  %d  %d", &cfg_pars.RxIllness_covariate_id,  &cfg_pars.RxIllness_prob, 
                                                &cfg_pars.RxIllness_duration, &cfg_pars.RxIllness_index_only);
                 if(cfg_pars.RxIllness_covariate_id > cfg_pars.n_time_dep_covariate)
                 {
                    printf("RxIllness_as_covariate=%d RxIllness_covariate_id=%d\n",  cfg_pars.RxIllness_as_covariate, cfg_pars.RxIllness_covariate_id);
                    printf("Please input a valid time dependent covariate for antiviral treatment indicator\n");
                    exit(0);
                 }
              }   
           }      
        ...
        if(cfg_pars.simulation == 1)
        {
           ...
           if(strcmp(st, "simulation-only") == 0)
              fscanf(file,"%d", &cfg_pars.simulation_only);
           
           if(strcmp(st, "output-simulation-data") == 0)
              fscanf(file,"%d", &cfg_pars.output_simulation_data);

           if(strcmp(st, "index-case-initiate-followup-in-simulation") == 0)
              fscanf(file,"%d", &cfg_pars.idx_initiated_followup_in_simulation);

           if(strcmp(st, "followup-duration-after-index-case-in-simulation") == 0)
              fscanf(file,"%d", &cfg_pars.followup_duration_after_idx_in_simulation);
        ...
        if(strcmp(st, "use-onset-days-of-index-cases-to-improve-estimation-of-b") == 0)
        {
           fscanf(file, "%d:", &cfg_pars.use_index_cases_to_improve_b);
           if(cfg_pars.use_index_cases_to_improve_b == 1)
           {
              fscanf(file, "%d", &cfg_pars.n_c2p_group); 
              if(cfg_pars.n_c2p_group <= 0)
              {
                  printf("If you want to utilize information provided by index cases for estimating b, each community should be affiliated with a C2P exposure group\n");
                  exit(0);
              } 
              else
              {
                 cfg_pars.c2p_group = (C2P_GROUP *) malloc((size_t) (cfg_pars.n_c2p_group  * sizeof(C2P_GROUP)));
                 for(i=0; i<cfg_pars.n_c2p_group; i++)
                 {
                    cfg_pars.c2p_group[i].earliest_idx_day_ill = MISSING;
                    cfg_pars.c2p_group[i].latest_idx_day_ill = MISSING;
                    cfg_pars.c2p_group[i].community_of_earliest_idx_day_ill = MISSING;
                    cfg_pars.c2p_group[i].community_of_latest_idx_day_ill = MISSING;
                 }
              }
           }
        }
...
int write_cfg_pars(FILE *file, CFG_PARS *cfg_pars)
{
  ...
  fprintf(file, "\n");
  fprintf(file, "# print-covariance-matrix\n");
  fprintf(file, "%d\n", cfg_pars->print_covariance);
